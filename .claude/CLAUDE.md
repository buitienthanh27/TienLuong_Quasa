# Claude AI Configuration — API_Sample (EcoTech2A)

## Auto-Invoke Skills (BẮT BUỘC)

**Mỗi khi user đặt câu hỏi hoặc yêu cầu task**, agent PHẢI tự động áp dụng:

1. **Karpathy Guidelines** — Invoke skill `karpathy-guidelines` từ `.claude/karpathy-skills/skills/karpathy-guidelines/SKILL.md`
2. **Superpowers Methodology** — Tuân thủ workflow brainstorm → plan → TDD → verify

### Checklist trước mỗi response:

- [ ] **Think Before Coding**: Đã nêu rõ assumptions? Có điểm nào chưa rõ cần hỏi?
- [ ] **Simplicity First**: Giải pháp có đơn giản nhất chưa? Có feature thừa không?
- [ ] **Surgical Changes**: Chỉ sửa đúng những gì được yêu cầu?
- [ ] **Goal-Driven**: Đã định nghĩa success criteria có thể verify?

**Nếu không chắc chắn → DỪNG LẠI và HỎI trước khi code.**
...
**Chi tiết:** Xem `.claude/rules/superpowers.md`
...
- Luôn đọc **tất cả file trong `.claude/rules/`** trước khi sinh code mới hoặc sửa code.
...
## Agent Behavior
- Trước khi tạo entity / DTO / service / controller mới: đọc lại các rule liên quan trong `.claude/rules/`.
